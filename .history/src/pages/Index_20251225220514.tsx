...
const Index = () => {
  ...
  // Ref pour l'autocomplétion Google
  const addressInputRef = useRef<HTMLInputElement>(null);
  const autocompleteRef = useRef<any>(null);

  // 1. SYNCHRONISATION : Quand on ouvre le menu, on remplit avec les valeurs actuelles
  useEffect(() => {
    if (showConfig) {
  ...
  // 2. AUTOCOMPLÉTIONS GOOGLE : Active la recherche d'adresse dans le menu config
  useEffect(() => {
    if (showConfig && addressInputRef.current && (window as any).google) {
      ...
      autocompleteRef.current = new googleMaps.places.Autocomplete(addressInputRef.current, {
        types: ['address'],
        componentRestrictions: { country: 'fr' }, // Restreindre à la France (optionnel)
      });
      ...
      if (status === 'OK' && results[0]) {
        ...
        setConfiguration(
          { 
            lat: loc.lat(), 
            lng: loc.lng(), 
            address: results[0].formatted_address // On sauvegarde l'adresse officielle Google
          },
          endTimeInput
        );
        setShowConfig(false);
      } else {
        alert("Adresse introuvable. Veuillez sélectionner une suggestion dans la liste.");
      }
    });
  };

  // Utiliser ma position GPS et trouver l'adresse correspondante
