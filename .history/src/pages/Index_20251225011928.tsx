...
import { Plus, Settings, Package, MapPin, Navigation, Search, Loader2, RefreshCw } from 'lucide-react';
...
const Index = () => {
  ...
  const [endAddressText, setEndAddressText] = useState(config.endLocation?.address || "");
  const [isGeocoding, setIsGeocoding] = useState(false);

  // Fonction pour gérer l'ajout avec alerte horaire
  ...
  // SAUVEGARDE DE LA CONFIGURATION (Adresse Texte -> Coordonnées GPS)
  const saveConfig = () => {
    // Si l'utilisateur n'a pas mis d'adresse, on vide la config
    ...
    // Si l'adresse a changé, on doit la géocoder via Google Maps
    setIsGeocoding(true);
    
    // On vérifie que Google Maps est chargé
    if (!(window as any).google?.maps) {
      alert("Erreur: La carte n'est pas encore chargée. Attendez quelques secondes.");
    ...
    geocoder.geocode({ address: endAddressText }, (results: any, status: string) => {
      ...
      if (status === 'OK' && results[0]) {
        ...
        // On sauvegarde la configuration avec les coordonnées trouvées
        setConfiguration(
          { 
            lat: loc.lat(), 
            lng: loc.lng(), 
            address: endAddressText // On garde le texte saisi pour l'affichage
          },
          endTimeInput
        );
        setShowConfig(false);
      } else {
        alert("Impossible de trouver cette adresse. Veuillez vérifier l'orthographe ou être plus précis (Ville, Code postal).");
      }
    });
  };

  // Fonction utilitaire pour utiliser la position GPS actuelle dans le champ texte
  const useCurrentPositionAsEnd = () => {
    if (driverLocation.lat) {
       // On fait l'inverse : Coordonnées -> Adresse (Geocoding inverse)
       setIsGeocoding(true);
       const geocoder = new (window as any).google.maps.Geocoder();
              ...
              <div className="grid gap-6 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Saisissez l'adresse exacte pour un calcul de retour précis.
                  </p>
                </div>
              </div>
        ...
        <div className={`lg:flex-1 lg:min-h-[600px] ${view === 'list' ? 'hidden md:block' : ''}`}>
          <div className="h-[50vh] lg:h-full relative rounded-2xl overflow-hidden shadow-card border border-border">
            ...
            {config.workEndTime && (
             ...
             {config.endLocation && (
              <div className="absolute top-4 left-4 bg-background/95 backdrop-blur px-3 py-2 rounded-lg shadow-lg text-xs font-medium border border-border flex items-center gap-2">
                <div className="bg-primary/10 p-1 rounded-full"><MapPin className="w-3 h-3 text-primary" /></div>
                <span className="max-w-[150px] truncate">{config.endLocation.address}</span>
              </div>
            )}
          </div>
        </div>
