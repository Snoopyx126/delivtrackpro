...
const Index = () => {
  ...
  // CORRECTION : On stocke directement les coordonnées quand l'utilisateur clique sur la liste
  ...
  useEffect(() => {
    if (showConfig) {
      ...
      // Si une config existe déjà, on la met dans l'état temporaire
  ...
  // Configuration Autocomplete Google
  useEffect(() => {
    if (showConfig && addressInputRef.current && (window as any).google) {
      ...
      autocompleteRef.current.addListener('place_changed', () => {
        ...
        // CORRECTION MAJEURE : On récupère les coords tout de suite
  ...
  const saveConfig = () => {
    ...
    // Cas 2: On a déjà sélectionné via la liste (Le cas qui plantait)
    ...
    // Cas 3: L'utilisateur a tapé à la main sans cliquer (Fallback)
    setIsGeocoding(true);
    ...
    if (driverLocation.lat) {
       ...
       geocoder.geocode({ location: latlng }, (results: any, status: string) => {
         ...
         if (status === 'OK' && results[0]) {
           ...
           // On met aussi à jour les coords sélectionnées
                    ...
                    <div className="relative flex-1">
                      ...
                      <Input 
                        ...
                        onChange={(e) => {
                            ...
                            // Si on tape, on reset la sélection précédente pour forcer un nouveau choix
